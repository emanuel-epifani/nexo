...
/// Minimal example of safe shared mutable state across threads in Rust.
///
/// Concepts shown:
/// - OS threads via `thread::spawn`
/// - Shared ownership via `Arc<T>`
/// - Mutual exclusion via `Mutex<T>`
/// - `move` closure to transfer ownership
/// - Joining threads via `JoinHandle`
fn main() {
    // Arc<T> = Atomic Reference Counted pointer
    // Allows multiple threads to own the same heap allocation safely.
    // The reference counter is updated atomically.
    let counter = Arc::new(Mutex::new(0));

    // Store JoinHandles so we can wait for threads to finish.
    let mut handles = vec![];

    for _ in 0..2 {
        // Clone the Arc, not the inner value.
        // This increments the atomic refcount.
        let counter = Arc::clone(&counter);

        // Spawn a new OS thread.
        // `move` is required to transfer ownership of `counter`
        // into the thread closure.
        handles.push(thread::spawn(move || {
            // Acquire the mutex lock.
            // This blocks the thread if another thread holds the lock.
            let mut value = counter.lock().unwrap();

            // We now have exclusive mutable access to the data.
            *value += 1;

            // When `value` (MutexGuard) goes out of scope,
            // the lock is automatically released (RAII).
        }));
    }
    ...
    for h in handles {
        ...
        // If the thread panicked, join() returns Err.
        h.join().unwrap();
    }

    // Final read of the shared value.
    // We must lock again to access the inner data.
