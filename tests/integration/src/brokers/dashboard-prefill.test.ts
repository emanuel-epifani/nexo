...
describe('DASHBOARD PREFILL - Complete Data Visualization', () => {
    ...
    it('should validate dashboard API response structure matches TypeScript types', async () => {
        // This test ensures the API response structure matches our TypeScript types
        ...
        // Store broker types
        const storeTotalKeys: number = brokers.store.total_keys;
        const storeExpiringKeys: number = brokers.store.expiring_keys;
        const storeKeys = brokers.store.keys;
        
        if (storeKeys.length > 0) {
            const firstKey = storeKeys[0];
            const keyName: string = firstKey.key;
            const valuePreview: string = firstKey.value_preview;
            const createdAt: string | null = firstKey.created_at;
            const expiresAt: string | null = firstKey.expires_at;
        }
        
        // Queue broker types
        const queues = brokers.queue.queues;
        if (queues.length > 0) {
            const firstQueue = queues[0];
            const queueName: string = firstQueue.name;
            const pendingCount: number = firstQueue.pending_count;
            const inflightCount: number = firstQueue.inflight_count;
            const scheduledCount: number = firstQueue.scheduled_count;
            const consumersWaiting: number = firstQueue.consumers_waiting;
            
            const messages = firstQueue.messages;
            if (messages.length > 0) {
                const firstMessage = messages[0];
                const messageId: string = firstMessage.id;
                const payloadPreview: string = firstMessage.payload_preview;
                const messageState: string = firstMessage.state;
                const priority: number = firstMessage.priority;
                const attempts: number = firstMessage.attempts;
                const nextDeliveryAt: string | null = firstMessage.next_delivery_at;
            }
        }
        
        // PubSub broker types
        const activeClients: number = brokers.pubsub.active_clients;
        const topicTree = brokers.pubsub.topic_tree;
        const topicName: string = topicTree.name;
        const topicFullPath: string = topicTree.full_path;
        const topicSubscribers: number = topicTree.subscribers;
        const retainedValue: string | null = topicTree.retained_value;
        const topicChildren = topicTree.children;
        
        const wildcardSubs = brokers.pubsub.wildcard_subscriptions;
        if (wildcardSubs.length > 0) {
            const firstWildcard = wildcardSubs[0];
            const pattern: string = firstWildcard.pattern;
            const clientId: string = firstWildcard.client_id;
        }
        
        // Stream broker types
        const totalTopics: number = brokers.stream.total_topics;
        const totalActiveGroups: number = brokers.stream.total_active_groups;
        const streamTopics = brokers.stream.topics;
        
        if (streamTopics.length > 0) {
            const firstStreamTopic = streamTopics[0];
            const streamTopicName: string = firstStreamTopic.name;
            const partitions = firstStreamTopic.partitions;
            
            if (partitions.length > 0) {
                const firstPartition = partitions[0];
                const partitionId: number = firstPartition.id;
                const partitionMessages = firstPartition.messages;
                const currentConsumers: string[] = firstPartition.current_consumers;
                const lastOffset: number = firstPartition.last_offset;
                
                if (partitionMessages.length > 0) {
                    const firstStreamMessage = partitionMessages[0];
                    const messageOffset: number = firstStreamMessage.offset;
                    const messageTimestamp: string = firstStreamMessage.timestamp;
                    const messagePayloadPreview: string = firstStreamMessage.payload_preview;
                }
            }
        }
        
        // If we reach here without TypeScript errors, types are aligned
