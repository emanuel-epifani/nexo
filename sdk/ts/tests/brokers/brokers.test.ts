...
describe('BROKER INTEGRATION', async () => {
    ...
    describe('QUEUE', () => {
        ...
        it('Should handle explicit NACK and persist failure reason in DLQ', async () => {
            ...
            // Max retries 1 means: 1st fail -> retry, 2nd fail -> DLQ.
            // But with NACK we can fail immediately.
            // Let's use maxRetries: 0 so 1st failure goes straight to DLQ.
            const q = await nexo.queue(qName).create({
                maxRetries: 0,
                visibilityTimeoutMs: 10000 // Long timeout to prove NACK works instantly
            });
