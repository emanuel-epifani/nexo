...
async function populateStore(nexo) {
  ...
  await nexo.store.map.set(
    "session:admin",
    { user_id: 1, role: "admin", active: true },
    { ttl: 3600 },
  );
  await nexo.store.map.set(
    "session:user:123",
    { user_id: 123, role: "user", active: true },
    { ttl: 1800 },
  );
  await nexo.store.map.set(
    "session:guest",
    { user_id: null, role: "guest", active: false },
    { ttl: 300 },
  );
  await nexo.store.map.set(
    "session:temp",
    { temp: true, expires_soon: true },
    { ttl: 10 },
  );
  ...
  await nexo.store.map.set(
    "text:footer",
    "© 2024 NEXO - High Performance Broker",
  );
...
async function populateQueue(nexo) {
  ...
  const videoSub = await videoQueue.subscribe(
    async () => {
...
async function populateStream(nexo) {
  ...
  const orderSub = await orderConsumer.subscribe(
  ...
  const systemSub = await systemConsumer.subscribe(
    "monitoring_group",
    (data) => {
