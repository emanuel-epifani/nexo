...
use crate::protocol::{encode_resp, parse_resp, Response};
...
async fn handle_connection(mut socket: TcpStream, kv: Arc<KvManager>) -> Result<(), String> {
    ...
    loop {
        ...
        // println!("[Socket] RESP format received from client: {}", String::from_utf8_lossy(&buffer[..]));
        ...
        let (resp_value, consumed) = match parse_resp(&buffer) {
            ...
            Err(e) => {
                // Send error response
                let error_resp = encode_resp(&Response::Error(format!("Protocol error: {}", e)));
                socket
        ...
        let args = match resp_value.into_string_array() {
            Ok(args) => args,
            Err(e) => {
                let error_resp = encode_resp(&Response::Error(format!("Invalid command format: {}", e)));
                socket
                    .write_all(&error_resp)
                    .await
                    .map_err(|e| format!("Failed to write error response: {}", e))?;
